...
const register = async (req, res) => {
  try {
    const { fullname, email, mobile, password } = req.payload;
    ...
    if (!user_role)
      return sendError(res, 400, "Registration is not available as user now.");
    ...
    if (avl_user && avl_user.email == email)
      return sendError(res, 409, "Email is already registered. Try to login.");
    if (avl_user && avl_user.mobile == mobile)
      return sendError(
        res,
        409,
        "Mobile number is already registered. Try to login."
      );
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (
      ...
      const { old_password, new_password, confirm_password } = req.payload;
      const avl_user = await Users.findOne({
        where: {
          id: user.id,
        },
      ...
      if (avl_user && !check_hash(old_password, avl_user.password))
        return sendError(res, 404, "Enter a valid old password.");
      if (old_password === new_password)
        return sendError(res, 400, "Old and new password cannot be same.");
